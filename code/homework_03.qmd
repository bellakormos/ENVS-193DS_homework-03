...
To evaluate the strength of the relationship between pickleweed biomass and soil salinity, I used a Spearman's rank correlation rho. We found a moderate relationship between soil salinity and pickleweed biomass (Spearman ρ = 0.59, S = 824, p = 0.003, α = 0.05), which suggests that pickleweed biomass tends to increase as soil salinity increases along the slough. Both the Pearson's product-moment correlation (r = 0.53) and Spearman's rank correlation rho(ρ = 0.59) found a moderate positive relationship between soil salinity and pickleweed biomass, which confirms that the result is robust and consistent with both correlation methods.
...
my_clean_data <- my_data |> # use cleaned data-set
  ...
  mutate(duration_decimal = as.numeric(duration)) # convert decimal duration to numeric
# base layer: ggplot
ggplot(
  data = my_clean_data,
  ...
  labs(title = "Analyzing productivity:\nTasks Completed vs. Study Duration",
       ...
       y = "Tasks Completed",
    subtitle = "Most recent observation: March 4, 2026") + # most recent date
  # add clean themes and remove minor grid lines
  theme_minimal() + 
  theme(panel.grid.minor = element_blank()) 
```

### Categorical variable visualization: Study Location vs Tasks completed
...
ggplot(data = my_clean_data, 
  ...
  geom_jitter(width = 0.1, # horizontal spread to avoid overlap
    ...
    labs(title = "Analyzing productivity:\nTasks Completed vs. Study Location",
           x = "Study Location",
           y = "Tasks Completed",
  subtitle = "Most recent observation: March 4, 2026") + # add subtitle with date
...
**Figure 1.** The positive relationship between study duration (hours) and tasks completed from all study sessions (most recent observation: March 4, 2026). These points are represented in a scatter-plot, and each green point represents a study session. There is a general tendency for more tasks to be completed during longer sessions, however, there is considerable spread in the data, particularly at shorter duration.

**Figure 2.** Distribution of tasks completed by study location (Café vs. Library) across all study sessions (most recent observation: March 4, 2026). Each black point is a study session. The library (in blue) has higher median (around 5 tasks) and more high-task outliers. Café study sessions (in gold) shows a tighter interquartile range and smaller median (around 3). 
...
[View Workshop Slides](https://docs.google.com/presentation/d/1SttqIWUE9gXhKkSHgmOoO1Xq_r0KEmFwASjNBVpp9Os/edit?slide=id.p#slide=id.p)
